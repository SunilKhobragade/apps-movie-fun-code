...
public class AlbumsController {
    ...
    public String uploadCover(@PathVariable long albumId, @RequestParam("file") MultipartFile uploadedFile) throws IOException {
        ...
        fileStore.put(new Blob(getCoverFileName(albumId), uploadedFile.getInputStream(), uploadedFile.getContentType()));
    ...
    public HttpEntity<byte[]> getCover(@PathVariable long albumId) throws IOException, URISyntaxException {
        FileStore fileStore = new FileStore();
        Optional<Blob> targetFile = fileStore.get(getCoverBlobName(albumId));
        Blob blob = targetFile.orElseGet(this::buildDefaultCoverBlob);
        //Path coverFilePath = getExistingCoverPath(albumId);
        byte[] imageBytes = readAllBytes(blob.);
